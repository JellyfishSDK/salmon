...
import { CoinMarketCapPriceProvider } from '../src'
...
describe('multi price fetch', () => {
  ...
  it('should fetch price from defichain dex using config', async () => {
    ...
    const priceManager = new PriceManager(config, new CoinMarketCapPriceProvider('API_TOKEN'))
    const prices = await priceManager.fetchAssetPrices()
    expect(prices[0].asset).toStrictEqual('BTC')
    expect(prices[0].price).toStrictEqual(new BigNumber('39633.853292688436'))
    expect(prices[1].asset).toStrictEqual('DOGE')
    expect(prices[1].price).toStrictEqual(new BigNumber('0.20917968613823'))
    expect(prices[2].asset).toStrictEqual('ETH')
    expect(prices[2].price).toStrictEqual(new BigNumber('2297.4646020723612'))
  })
})
