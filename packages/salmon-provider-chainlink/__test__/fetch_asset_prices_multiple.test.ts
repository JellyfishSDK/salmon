...
describe('inverse price fetch', () => {
  ...
  it('should fetch inverse price from chainlink using config', async () => {
    ...
    const config: PriceSourceConfig = {
      symbols: ['BTC_TEST_INVERSE']
    }

    const priceManager = new PriceManager(config, new ChainlinkPriceProvider('API_TOKEN'))
    const prices = await priceManager.fetchAssetPrices()
    expect(prices[0].asset).toStrictEqual('BTC_TEST_INVERSE')
    expect(prices[0].price).toStrictEqual((new BigNumber(1.0)).dividedBy(46017.61999158))
    expect(prices[0].timestamp).toStrictEqual(new BigNumber(1631609928000))
  })
})
